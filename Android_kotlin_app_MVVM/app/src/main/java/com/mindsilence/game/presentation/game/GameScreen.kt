package com.mindsilence.game.presentation.game // Training Route + Screen: ring, Start/Thought, summary dialog.
...
import androidx.hilt.navigation.compose.hiltViewModel // Hilt GameViewModel; no handwritten factory.
...
import com.mindsilence.game.domain.model.GamePhase // Idle vs Running for layout.
import com.mindsilence.game.domain.model.SessionSummary // Dialog preview payload.
...
fun GameRoute( // AppRoute supplies nav lambdas; repo comes from Hilt.
    ...
    viewModel: GameViewModel = hiltViewModel(), // Destroyed when this route leaves composition if the store is cleared.
) { // Start GameRoute body.
    val state = viewModel.state.collectAsStateWithLifecycle().value // Phase, level, summary.

    BackHandler { viewModel.onLeaveTraining() } // System Back → menu, not finish Activity.
    ...
    DisposableEffect(lifecycleOwner) { // Register while this route is composed.
        val observer = LifecycleEventObserver { _, event -> // Map lifecycle to ViewModel methods.
            when (event) { // Only stop/start; pause UI does not exist.
                Lifecycle.Event.ON_STOP -> viewModel.onAppBackgrounded() // Pause tick, keep Running.
                Lifecycle.Event.ON_START -> viewModel.onAppForegrounded() // Resume tick if needed.
    ...
    GameScreen( // Stateless layout.
        ...
        onStartClick = { viewModel.onStart() }, // Idle only (VM ignores Running).
        onThoughtClick = { viewModel.onThought() }, // Running only (VM ignores Idle).
        onDismissSessionSummary = { viewModel.onDismissSessionSummary() }, // Close dialog.
        onOpenHighScores = { viewModel.onOpenHighScores() }, // Dialog Highscore.
        ...
        Column( // Top spacer, focus, buttons.
            ...
            Row( // Start left, Thought right.
                ...
                Button( // Start a silence attempt.
                    onClick = onStartClick, // onStart(); VM ignores Running.
                ...
                Button( // End the attempt when a thought appears.
                    onClick = onThoughtClick, // onThought(); VM ignores Idle.
