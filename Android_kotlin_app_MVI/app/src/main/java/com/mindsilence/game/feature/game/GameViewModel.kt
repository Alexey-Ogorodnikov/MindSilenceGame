package com.mindsilence.game.feature.game // Session logic: timer, Thought, persist, effects.
 ...
 */
class GameViewModel( // All session rules live here, not in GameScreen.
    private val progressRepository: GameProgressRepository = InMemoryGameProgressRepository(), // Tests default to memory; device uses prefs.
) : ViewModel() { // Scoped to GameRoute; destroyed when highscores is shown.
    ...
    fun onEvent(event: GameUiEvent) { // Single entry from GameRoute/GameScreen.
        when (event) { // Ignore illegal Start/Thought via the private handlers.
            GameUiEvent.Start -> startSession() // Idle → Running at level 1.
            GameUiEvent.Thought -> onThought() // End the attempt; ignore if Idle.
            GameUiEvent.DismissSessionSummary -> dismissSessionSummary() // Close the dialog only.
            GameUiEvent.OpenHighScores -> openHighScores() // Close summary then navigate.
            GameUiEvent.LeaveTraining -> leaveTraining() // Back to menu.
            GameUiEvent.AppBackgrounded -> onAppBackgrounded() // ON_STOP: pause tick, keep phase Running.
            GameUiEvent.AppForegrounded -> onAppForegrounded() // ON_START: resume tick if it was Running.
        } // End when.
    } // End onEvent.

    private fun startSession() { // Start is ignored while already Running.
    ...
    private fun onThought() { // Thought in Idle is ignored.
        ...
        val bestToday = progressRepository.recordSession(levelReached, totalSeconds) // Persist today; returns today's best.
    ...
    private fun dismissSessionSummary() { // Close dialog without navigating.
        _state.update { it.copy(sessionSummary = null) } // Stay on Idle training.
    } // End dismissSessionSummary.

    private fun openHighScores() { // Dialog Highscore button.
        ...
        viewModelScope.launch { // Navigate after closing the summary.
            _effects.send(GameUiEffect.NavigateToHighScores) // AppRoute sets showHighScores; GameViewModel is destroyed.
        } // End navigate launch.
    } // End openHighScores.

    private fun leaveTraining() { // System Back on training.
        viewModelScope.launch { // Parent clears inTraining.
            _effects.send(GameUiEffect.NavigateBackToMenu) // AppUiEvent.LeaveTraining.
        } // End leave launch.
    } // End leaveTraining.

    private fun onAppBackgrounded() { // ON_STOP from GameRoute.
    ...
    } // End onAppBackgrounded.

    private fun onAppForegrounded() { // ON_START from GameRoute.
